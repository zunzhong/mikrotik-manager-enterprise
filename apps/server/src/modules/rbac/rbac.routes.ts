...
import { rbacAnyGuard, rbacGuard } from './rbac.guard.js';
...
const rbacAssignmentPermissions = ['rbac:assign', 'role.write'] as RbacPermission[];
  ...
  app.post(
    '/api/v1/rbac/users/:userId/roles',
    {
      preHandler: [
        ...
        rbacAnyGuard(rbacAssignmentPermissions, 'RBAC role assignment permission is required'),
      ],
    },
    async (request, reply) => {
      const params = userParamsSchema.parse(request.params);
