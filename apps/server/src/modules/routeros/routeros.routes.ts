...
export async function routerosRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/api/v1/routeros/probe', async (request, reply) => {
    try {
      return {
        success: true,
        data: await routerOsSdkAdapter.probe(routerOsProbeInputSchema.parse(request.body)),
      };
    } catch (error) {
      reply.code(400);
      return {
  ...
  app.post('/api/v1/routeros/inventory', async (request, reply) => {
    try {
      return {
        success: true,
        data: await routerOsSdkAdapter.inventory(routerOsProbeInputSchema.parse(request.body)),
      };
    } catch (error) {
      reply.code(400);
      return {
        success: false,
        error: {
          code: 'ROUTEROS_INVENTORY_FAILED',
          message: error instanceof Error ? error.message : 'RouterOS inventory failed',
        },
      };
    }
  });
  ...
  app.post('/api/v1/routeros/sync', async (request, reply) => {
    try {
      return {
        success: true,
        data: await deviceSyncService.collect(routerOsProbeInputSchema.parse(request.body)),
      };
    } catch (error) {
      reply.code(400);
      return {
        success: false,
        error: {
          code: 'ROUTEROS_SYNC_FAILED',
          message: error instanceof Error ? error.message : 'RouterOS sync failed',
        },
      };
    }
  });
  ...
  app.post('/api/v1/routeros/identity', async (request, reply) => {
    try {
      return {
        success: true,
        data: await routerOsSdkAdapter.identity(routerOsProbeInputSchema.parse(request.body)),
      };
    } catch (error) {
      reply.code(400);
      return {
  ...
  app.post('/api/v1/routeros/resource', async (request, reply) => {
    try {
      return {
        success: true,
        data: await routerOsSdkAdapter.resource(routerOsProbeInputSchema.parse(request.body)),
      };
    } catch (error) {
      reply.code(400);
      return {
  ...
  app.post('/api/v1/routeros/routerboard', async (request, reply) => {
    try {
      return {
        success: true,
        data: await routerOsSdkAdapter.routerboard(routerOsProbeInputSchema.parse(request.body)),
      };
    } catch (error) {
      reply.code(400);
      return {
