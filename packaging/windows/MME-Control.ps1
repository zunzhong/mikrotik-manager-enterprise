...
function Initialize-Environment {
  ...
  if (Test-Path $ConfigFile) {
    ...
    if ($existingConfig -match '(?m)^APP_VERSION=') {
      $existingConfig = [Text.RegularExpressions.Regex]::Replace($existingConfig, '(?m)^APP_VERSION=.*$', 'APP_VERSION=4.1.7')
    } else {
      $existingConfig = $existingConfig.TrimEnd() + "`r`nAPP_VERSION=4.1.7`r`n"
    }
