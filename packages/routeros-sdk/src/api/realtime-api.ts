...
function mapRealtimeTraffic(row: RouterOsRecord): RouterOsRealtimeTrafficSample {
  return {
    name: row.name,
    rxBitsPerSecond: row.rxBitsPerSecond,
    txBitsPerSecond: row.txBitsPerSecond,
    rxPacketsPerSecond: row.rxPacketsPerSecond,
    txPacketsPerSecond: row.txPacketsPerSecond,
    rxDropsPerSecond: row.rxDropsPerSecond,
    txDropsPerSecond: row.txDropsPerSecond,
    rxErrorsPerSecond: row.rxErrorsPerSecond,
    txErrorsPerSecond: row.txErrorsPerSecond,
...
export class RealtimeApi {
  ...
  public wirelessRegistration(): Promise<RouterOsWirelessRegistration[]> {
    return this.runner.print('/interface/wireless/registration-table/print') as Promise<RouterOsWirelessRegistration[]>;
  }

  public wifiRegistration(): Promise<RouterOsWirelessRegistration[]> {
    return this.runner.print('/interface/wifi/registration-table/print') as Promise<RouterOsWirelessRegistration[]>;
  }
}
