...
export class EthernetApi {
  ...
  public async list(): Promise<RouterOsInterface[]> {
    return mapList(await this.runner.print('/interface/ethernet/print'), mapInterface);
  }
  public async get(id: string): Promise<RouterOsInterface | undefined> {
    return mapList(
      await this.runner.print('/interface/ethernet/print', { queries: { '.id': id } }),
      mapInterface,
    )[0];
  }
