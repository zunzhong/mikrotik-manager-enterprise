...
import {
  mapBandwidthTestResult,
  mapPingResult,
  mapTorchEntry,
  mapTracerouteHop,
} from '../mappers/diagnostics.mapper.js';
import type {
...
export class DiagnosticsApi {
  ...
  public async ping(input: {
    address: string;
    count?: string | number;
    interval?: string;
    size?: string | number;
  }): Promise<RouterOsPingResult[]> {
    const rows = await this.runner.print('/ping', {
      attributes: {
  ...
  public async traceroute(input: {
    address: string;
    count?: string | number;
    timeout?: string;
  }): Promise<RouterOsTracerouteHop[]> {
    const rows = await this.runner.print('/tool/traceroute', {
      attributes: { address: input.address, count: input.count, timeout: input.timeout },
      timeoutMs: 30000,
    });
    return rows.map(mapTracerouteHop);
  }
  public async torch(input: {
    interface: string;
    srcAddress?: string;
    dstAddress?: string;
    port?: string | number;
    ...
    const rows = await this.runner.print('/tool/torch', {
      attributes: {
  ...
  public async bandwidthTest(input: {
    address: string;
    user?: string;
    password?: string;
    direction?: string;
    ...
    const rows = await this.runner.print('/tool/bandwidth-test', {
      attributes: {
        address: input.address,
        user: input.user,
