...
export function parseReply(sentence: string[]): RouterOsReply {
  ...
  for (const word of sentence.slice(1)) {
    ...
    const second = word.indexOf('=', 1);
    if (second > 0) attributes[word.slice(1, second)] = word.slice(second + 1);
  }
  return { type, attributes, words: sentence, tag };
}
export function firstData(replies: RouterOsReply[]): Record<string, string> {
  return replies.find((r) => r.type === '!re')?.attributes ?? {};
}
